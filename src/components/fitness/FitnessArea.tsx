...
import { Plus, Flame, Dumbbell, ChevronLeft, ChevronRight, X, Clock, Calendar } from "lucide-react";
...
export function FitnessArea() {
  ...
  const [activityType, setActivityType] = useState<FitnessActivityType>("workout");
  const [newActivity, setNewActivity] = useState({ name: "", duration: "", notes: "" });
  ...
  const activitiesForDate = useMemo(() => {
    ...
    return state.fitnessActivities.filter(a => a.date === dateStr);
  ...
  const activityDays = useMemo(() => {
    ...
    state.fitnessActivities.forEach(activity => {
      if (!days[activity.date]) {
        days[activity.date] = { neat: 0, workout: 0 };
      }
      if (activity.type === "neat") {
        days[activity.date].neat++;
      } else {
        days[activity.date].workout++;
      }
    });
    return days;
  ...
  const handleAddActivity = () => {
    ...
    setNewActivity({ name: "", duration: "", notes: "" });
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <Dialog open={isAddingActivity} onOpenChange={setIsAddingActivity}>
                  ...
                  <DialogContent className="max-w-sm">
                    ...
                    <Tabs value={activityType} onValueChange={(v) => setActivityType(v as FitnessActivityType)}>
                      <TabsList className="w-full">
                        <TabsTrigger value="workout" className="flex-1 gap-1">
                          <Dumbbell className="w-4 h-4" />
                          Entrenamiento
                        </TabsTrigger>
                        <TabsTrigger value="neat" className="flex-1 gap-1">
                    ...
                    <div className="space-y-4 mt-4">
                      <Input
                        ...
                        placeholder={activityType === "workout" ? "Ej: Pesas, Cardio..." : "Ej: Caminar, Subir escaleras..."}
                      />
                      ...
                      <Textarea
                        ...
                        rows={2}
                      />
                      <Button onClick={handleAddActivity} disabled={!newActivity.name.trim()} className="w-full">
                        Guardar Actividad
                      </Button>
                    </div>
                  </DialogContent>
